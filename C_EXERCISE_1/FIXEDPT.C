...
int num2;
{
    ...
    /*
        Subtract the integers, then subtract the decimals, then handle
        decimal underflow
    */
    integer1 -= integer2;
    decimal1 -= decimal2;
...
int num2;
{
    ...
    /*
        ...
        d_f = decimal % FPTMAX
   */
    decimal1 = (integer1 * decimal2) + (integer2 * decimal1) +
               (decimal1 * decimal2) / FPTMAX;
    ...
    for (c = getchar(); c != 10 && i < MAXNUMLEN; c = getchar(), i++) {
        ...
        if(decimalStart == 0) {
            ...
            /*
                ...
                But since we can assume that the user will only be able to
                enter 2 decimal places, we can just use a branch, which will
                be faster:
            */
            if(i == decimalStart + 1) {
                decimal += (c - '0') * 10;
